...
# UR5e arm joint / actuator names (relative to a robot's prefix). Used to locate
# arm DOFs by name, since attaching a gripper inserts extra joints/actuators and
# breaks the old "6 contiguous DOFs per robot" assumption.
ARM_JOINT_NAMES = (
    "shoulder_pan_joint",
    "shoulder_lift_joint",
    "elbow_joint",
...
# Prefix applied to the gripper when it is attached onto each arm's tool site.
GRIPPER_PREFIX = "grip_"
GRIPPER_ACTUATOR_NAME = "grip_fingers_actuator"
# All 8 gripper joints (relative to a robot's prefix). Used to interpolate the
# gripper pose in kinematic mode, where the actuator/tendon is not integrated.
GRIPPER_JOINT_NAMES = (
...
# 2F85 actuator control range (0 = fully open, 255 = fully closed).
GRIPPER_CTRL_OPEN = 0.0
GRIPPER_CTRL_CLOSED = 255.0
...
def build_model() -> mujoco.MjModel:
    ...
    gripper_mjcf_path = Path(robotiq_2f85_mj_description.MJCF_PATH)
...
"""
    ...
    for prefix, offset, incline_deg, yaw_deg in zip(
        ...
        """
        Attach a Robotiq 2F85 gripper onto the arm's tool site so it moves with
        the wrist. ``attach_body`` prefixes every gripper element with
        ``GRIPPER_PREFIX``; when the whole arm is later attached under ``prefix``
        the names become unique (e.g. ``left_grip_left_driver_joint``).
        """
        gripper_spec = mujoco.MjSpec.from_file(str(gripper_mjcf_path))
        tool_site = robot_spec.site(EE_SITE_NAME)
        tool_site.attach_body(
            gripper_spec.worldbody.first_body(), GRIPPER_PREFIX, ""
        )
...
class SimState:
    ...
    def __init__(self, sliders_available: bool = True):
        ...
        # Pending per-robot gripper openings (0.0 = open, 1.0 = closed).
        self._pending_gripper: dict[str, float] = {}
    ...
    def queue_gripper(self, robot: str, opening: float) -> None:
        with self._lock:
            self._pending_gripper[robot] = float(np.clip(opening, 0.0, 1.0))

    def request_home(self) -> None:
        with self._lock:
            self._home_requested = True
            self._pending.clear()
            self._pending_gripper.clear()
    ...
    def take_commands(
        ...
        with self._lock:
            ...
            self._pending_gripper = {}
            home = self._home_requested
            self._home_requested = False
            return pending, gripper, home
...
class GripperCommand(BaseModel):
    ...
    position: float = Field(
        ..., ge=0.0, le=1.0, description="0.0 = open, 1.0 = fully closed."
    )


class GripperBatchCommand(BaseModel):
...
def create_app(state: SimState) -> FastAPI:
    ...
    @app.post("/gripper")
    def post_gripper(cmd: GripperCommand) -> dict:
        if cmd.robot not in ROBOT_NAME_TO_PREFIX:
            raise HTTPException(
                status_code=422,
                detail=f"Unknown robot '{cmd.robot}'. Use 'left' or 'right'.",
            )
        state.queue_gripper(cmd.robot, cmd.position)
        return {"status": "ok", "robot": cmd.robot, "position": cmd.position}

    @app.post("/gripper/batch")
    def post_gripper_batch(cmd: GripperBatchCommand) -> dict:
        applied = []
        if cmd.left is not None:
            state.queue_gripper("left", cmd.left)
            applied.append("left")
        if cmd.right is not None:
            state.queue_gripper("right", cmd.right)
            applied.append("right")
        if not applied:
            raise HTTPException(status_code=422, detail="No gripper targets provided.")
        return {"status": "ok", "applied": applied}

    @app.post("/gripper/open")
    def post_gripper_open() -> dict:
        for robot in ROBOT_NAME_TO_PREFIX:
            state.queue_gripper(robot, 0.0)
        return {"status": "ok", "position": 0.0}

    @app.post("/gripper/close")
    def post_gripper_close() -> dict:
        for robot in ROBOT_NAME_TO_PREFIX:
            state.queue_gripper(robot, 1.0)
        return {"status": "ok", "position": 1.0}
...
class Simulation:
    def __init__(
        ...
        # Arm joint qpos addresses and actuator ids per robot, looked up by name
        # so the layout stays correct even though each attached gripper inserts
        # extra joints/actuators between the two arms' blocks.
        self.arm_qadr = [
            [int(self.model.joint(prefix + name).qposadr[0]) for name in ARM_JOINT_NAMES]
            for prefix in ROBOT_PREFIXES
        ]
        self.arm_act_ids = [
            [int(self.model.actuator(prefix + name).id) for name in ARM_ACTUATOR_NAMES]
            for prefix in ROBOT_PREFIXES
        ]

        # Gripper actuator ids and joint qpos addresses per robot. In physics
        # mode a single actuator (ctrl in [0, 255]) drives each gripper; in
        # kinematic mode we interpolate the joints between calibrated poses.
        self.gripper_act_ids = [
            int(self.model.actuator(prefix + GRIPPER_ACTUATOR_NAME).id)
            for prefix in ROBOT_PREFIXES
        ]
        self.gripper_qadr = [
            [int(self.model.joint(prefix + name).qposadr[0]) for name in GRIPPER_JOINT_NAMES]
            for prefix in ROBOT_PREFIXES
        ]
        # Filled by ``_calibrate_gripper`` (kinematic mode only).
        self._gripper_open_qpos: Optional[list[list[float]]] = None
        self._gripper_closed_qpos: Optional[list[list[float]]] = None
        ...
        if not self.physics:
            # Precompute the open/closed gripper joint poses so kinematic mode
            # can interpolate them (the tendon/actuator is not integrated there).
    ...
    def _reset_home(self):
        for robot_index in range(len(ROBOT_PREFIXES)):
            # Arm joints are no longer a fixed ``robot_index * 6`` block once a
            # gripper is attached, so address each arm joint by its qpos index.
            for joint_index, qadr in enumerate(self.arm_qadr[robot_index]):
                self.data.qpos[qadr] = HOME_QPOS[joint_index]
        ...
        if self.physics:
            ...
            self.data.qvel[:] = 0.0
            for robot_index in range(len(ROBOT_PREFIXES)):
                for joint_index, act_id in enumerate(self.arm_act_ids[robot_index]):
                    self.data.ctrl[act_id] = HOME_QPOS[joint_index]
            for act_id in self.gripper_act_ids:
                self.data.ctrl[act_id] = GRIPPER_CTRL_OPEN

        mujoco.mj_forward(self.model, self.data)
        self._prev_qpos[:] = self.data.qpos

    def _calibrate_gripper(self) -> None:
        """Settle the grippers fully open and fully closed under physics once, so
        kinematic mode can linearly interpolate the joint poses in between."""
        data = mujoco.MjData(self.model)
        for robot_index in range(len(ROBOT_PREFIXES)):
            for joint_index, qadr in enumerate(self.arm_qadr[robot_index]):
                data.qpos[qadr] = HOME_QPOS[joint_index]
            for joint_index, act_id in enumerate(self.arm_act_ids[robot_index]):
                data.ctrl[act_id] = HOME_QPOS[joint_index]

        def settle(ctrl_value: float) -> list[list[float]]:
            for act_id in self.gripper_act_ids:
                data.ctrl[act_id] = ctrl_value
            for _ in range(2000):
                mujoco.mj_step(self.model, data)
            return [
                [float(data.qpos[a]) for a in self.gripper_qadr[robot_index]]
                for robot_index in range(len(ROBOT_PREFIXES))
            ]

        self._gripper_open_qpos = settle(GRIPPER_CTRL_OPEN)
        self._gripper_closed_qpos = settle(GRIPPER_CTRL_CLOSED)

    def _set_robot_gripper_qpos(self, robot_index: int, opening: float) -> None:
        """Interpolate one gripper's joints between the calibrated open/closed
        poses. No-op when calibration is unavailable (e.g. physics mode)."""
        if self._gripper_open_qpos is None or self._gripper_closed_qpos is None:
            return
        opening = float(np.clip(opening, 0.0, 1.0))
        open_vals = self._gripper_open_qpos[robot_index]
        closed_vals = self._gripper_closed_qpos[robot_index]
        for k, qadr in enumerate(self.gripper_qadr[robot_index]):
            self.data.qpos[qadr] = open_vals[k] + opening * (closed_vals[k] - open_vals[k])

    def _set_gripper_qpos(self, opening: float) -> None:
        for robot_index in range(len(ROBOT_PREFIXES)):
            self._set_robot_gripper_qpos(robot_index, opening)

    def _gripper_opening(self, robot_index: int) -> float:
        """
        Normalized gripper opening in [0, 1] derived from the driver joint.
        ...
        <general class="2f85" name="fingers_actuator" ... ctrlrange="0 255" ... />
        """
        driver_qadr = self.gripper_qadr[robot_index][0]
        return float(np.clip(self.data.qpos[driver_qadr] / 0.8, 0.0, 1.0))

    def _build_snapshot(self, velocity: np.ndarray) -> dict:
        robots = {}
        for robot_index, prefix in enumerate(ROBOT_PREFIXES):
            name = prefix.rstrip("_")
            qadr = self.arm_qadr[robot_index]
            ee_site_id = self.ee_site_ids[robot_index]
            robots[name] = {
                "positions": [float(self.data.qpos[a]) for a in qadr],
                "velocities": [
                    float(velocity[a]) for a in qadr
                ],  #  come from MuJoCo’s qvel; in kinematic mode your code estimates them from finite differences.
                "ee_position": self.data.site_xpos[ee_site_id].tolist(),
                "ee_xmat": self.data.site_xmat[ee_site_id].tolist(),
                "gripper": self._gripper_opening(robot_index),
            }
    ...
    def _apply_commands(self) -> None:
        pending, gripper, home = self.state.take_commands()
        if home:
            self._reset_home()
        ...
        for robot_name, positions in pending.items():
            ...
            if self.physics:
                ...
                for joint_index, act_id in enumerate(self.arm_act_ids[robot_index]):
                    self.data.ctrl[act_id] = positions[joint_index]
            else:
                for joint_index, qadr in enumerate(self.arm_qadr[robot_index]):
                    self.data.qpos[qadr] = positions[joint_index]

        for robot_name, opening in gripper.items():
            prefix = ROBOT_NAME_TO_PREFIX[robot_name]
            robot_index = ROBOT_PREFIXES.index(prefix)
            if self.physics:
                # Drive the gripper actuator (0 = open, 255 = closed).
                self.data.ctrl[self.gripper_act_ids[robot_index]] = (
                    GRIPPER_CTRL_OPEN + opening * (GRIPPER_CTRL_CLOSED - GRIPPER_CTRL_OPEN)
                )
            else:
                self._set_robot_gripper_qpos(robot_index, opening)

        if not self.physics and (pending or gripper or home):
...
class ManualJointPanel:
    ...
    def __init__(self, sim: Simulation, state: SimState):
        ...
        self._targets = {
            prefix.rstrip("_"): np.array(
                [sim.data.qpos[a] for a in sim.arm_qadr[robot_index]],
                dtype=np.float64,
            )
            for robot_index, prefix in enumerate(ROBOT_PREFIXES)
        }
        ...
        for robot_index, prefix in enumerate(ROBOT_PREFIXES):
            ...
            for joint_index in range(DOF_PER_ROBOT):
                qpos_index = sim.arm_qadr[robot_index][joint_index]
                joint = sim.model.joint(prefix + ARM_JOINT_NAMES[joint_index])
