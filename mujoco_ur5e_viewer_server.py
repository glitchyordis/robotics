...
_XML = Path(__file__).parent / "mink" / "examples" / "universal_robots_ur5e" / "scene.xml"
UDP_HOST = "127.0.0.1"
UDP_PORT = 5005
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            if latest_packet is not None and len(latest_packet) == PACKET_SIZE:
                ...
                data.qpos[:] = q
                # data.mocap_quat[target_mocap_id] = target[:4]
                # data.mocap_pos[target_mocap_id] = target[4:]
                mujoco.mj_forward(model, data)
